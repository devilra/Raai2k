...
const AboutBanner = require("../../models/AdminAboutModels/AboutBanner");

// 1. அனைத்து Solution Banner-களையும் பெறுதல் (READ All - List Mode)
// இது Admin UI-இல் List-ஐக் காட்டப் பயன்படும்.

exports.getAboutBanners = async (req, res) => {
  try {
    const banners = await AboutBanner.findAll({
    ...
    return res.status(500).json({ message: "About Banners fetch failed" });
  }
};
...
exports.getPublishedAboutBanner = async (req, res) => {
  try {
    // 💡 மாற்றப்பட்ட வரி: findOne-க்கு பதிலாக findAll பயன்படுத்தப்பட்டுள்ளது
    const publishedBanners = await AboutBanner.findAll({
      where: {
    ...
    console.error("Error fetching published About banners:", error);
    return res
      .status(500)
      .json({ message: "Published About Banners fetch failed" });
  }
};
...
exports.createAboutBanner = async (req, res) => {
  ...
  try {
    ...
    // அடிப்படை validation
    if (!title) {
      return res.status(400).json({ message: "Title  are mandatory." });
    }
    ...
    const newBanner = await AboutBanner.create({
      title,
      description,
...
exports.updateAboutBanner = async (req, res) => {
  ...
  try {
    ...
    if (!banner) {
      return res.status(404).json({ message: "About Banner not found" });
    }
    ...
    return res.status(500).json({ message: "About Banner Update Error" });
  }
};
...
exports.deleteAboutBanner = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "About Banner deleted successfully" });
  } catch (error) {
    console.error("Error deleting About banner:", error);
    return res.status(500).json({ message: "About Banner deleted Error" });
  }
};
