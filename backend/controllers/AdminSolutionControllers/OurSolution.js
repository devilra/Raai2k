const OurSolution = require("../../models/AdminSolutionModels/OurSolution");

// --- 1. Welcome Content-ஐ உருவாக்குதல் (POST /api/admin/welcome-content) ---
exports.createOurSolution = async (req, res) => {
  ...
  try {
    ...
    const newContent = await OurSolution.create({
      mainHeading,
      subText,
      order,
      sectionActive,
...
exports.getOurSolutionById = async (req, res) => {
  try {
    const content = await OurSolution.findByPk(req.params.id);
...
exports.updateOurSolution = async (req, res) => {
  try {
    const { mainHeading, subText, order, sectionActive } = req.body;
    ...
    const content = await OurSolution.findByPk(contentId);
...
exports.deleteOurSolution = async (req, res) => {
  try {
    const contentId = req.params.id;
    const content = await OurSolution.findByPk(contentId);
...
exports.getAllOurSolution = async (req, res) => {
  try {
    // Admin Listing-க்காக அனைத்து entry-களையும் பெறுக
    const contents = await OurSolution.findAll({
      order: [["order", "ASC"]],
    });
