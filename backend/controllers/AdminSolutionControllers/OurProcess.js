const OurProcess = require("../../models/AdminSolutionModels/OurProcess");

// --- 1. Welcome Content-ஐ உருவாக்குதல் (POST /api/admin/welcome-content) ---
exports.createOurProcess = async (req, res) => {
  ...
  try {
    ...
    const newContent = await OurProcess.create({
      mainHeading,
      subText,
      order,
      sectionActive,
...
exports.getOurProcessById = async (req, res) => {
  try {
    const content = await OurProcess.findByPk(req.params.id);
...
exports.updateOurProcess = async (req, res) => {
  try {
    const { mainHeading, subText, order, sectionActive } = req.body;
    ...
    const content = await OurProcess.findByPk(contentId);
...
exports.deleteOurProcess = async (req, res) => {
  try {
    const contentId = req.params.id;
    const content = await OurProcess.findByPk(contentId);
...
exports.getAllOurProcess = async (req, res) => {
  try {
    // Admin Listing-க்காக அனைத்து entry-களையும் பெறுக
    const contents = await OurProcess.findAll({
      order: [["order", "ASC"]],
    });
