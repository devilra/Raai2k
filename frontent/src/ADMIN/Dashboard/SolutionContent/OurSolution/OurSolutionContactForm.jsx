...
const OurSolutionContactForm = ({
  isOpen,
  onClose,
  solutionData,
  setAlert,
}) => {
  const { isLoading } = useSelector((state) => state.ourSolution);
  const [formData, setFormData] = useState(initialFormData);
  const dispatch = useDispatch();

  //console.log(formData);

  const isEditMode = !!solutionData; // slideData இருந்தால் Edit Mode

  useEffect(() => {
    if (solutionData) {
      setFormData({
        mainHeading: solutionData.mainHeading || "",
        subText: solutionData.subText || "",
        order: solutionData.order || 1,
        sectionActive: solutionData.sectionActive,
      });
    } else {
  ...
  const handleSubmit = (e) => {
    ...
    const action = isEditMode
      ? updateOurSolution({ id: solutionData.id, data: dataToSave })
      : createOurSolution(dataToSave); // 💡 createSlide action சேர்க்கப்பட்டுள்ளது
...
export default OurSolutionContactForm;
