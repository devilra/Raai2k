...
import {
  ...
  fetchOurProcess,
  updateOurProcess,
} from "../../../../redux/AdminSolutionSlices/OurProcessSlice";
...
const OurProcessContactForm = ({ isOpen, onClose, processData, setAlert }) => {
  const { isLoading } = useSelector((state) => state.ourProcess);
  const [formData, setFormData] = useState(initialFormData);
  const dispatch = useDispatch();

  //console.log(formData);

  const isEditMode = !!processData; // slideData இருந்தால் Edit Mode

  useEffect(() => {
    if (processData) {
      setFormData({
        mainHeading: processData.mainHeading || "",
        subText: processData.subText || "",
        order: processData.order || 1,
        sectionActive: processData.sectionActive,
      });
    } else {
  ...
  const handleSubmit = (e) => {
    ...
    const action = isEditMode
      ? updateOurProcess({ id: processData.id, data: dataToSave })
      : createOurProcess(dataToSave); // 💡 createSlide action சேர்க்கப்பட்டுள்ளது

    dispatch(action)
      .unwrap() // Thunk Success/Failure-ஐ கையாள
      .then(() => {
        dispatch(fetchOurProcess());
...
export default OurProcessContactForm;
