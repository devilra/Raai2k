...
import {
  deleteOurProcess,
  fetchOurProcess,
  resetOurProcessState,
} from "../../../../redux/AdminSolutionSlices/OurProcessSlice";
import OurProcessContactForm from "./OurProcessContactForm";
...
const OurProcessSection = () => {
  ...
  const {
    ourProcess, // அனைத்து contents-களும் (List)
    isLoading,
    isError,
    isSuccess,
    message,
  } = useSelector((state) => state.ourProcess);
  ...
  if (isLoading && ourProcess.length === 0) {
  ...
  return (
    <div>
        ...
        <div className="bg-white shadow-lg rounded-xl overflow-hidden">
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {ourProcess.length > 0 ? (
                ourProcess
                  .filter((process) => process && process.order)
                  .map((process) => (
                    <tr key={process.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <Button
                          onClick={() => handleEditWelcome(process)}
      ...
      {/* Create/Edit Modal */}
      <OurProcessContactForm
        isOpen={isModalOpen}
        onClose={handleCloseModal}
        processData={currentWelcome}
        setAlert={setAlert}
      />
