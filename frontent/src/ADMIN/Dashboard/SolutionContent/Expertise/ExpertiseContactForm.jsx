...
import {
  ...
  fetchExpertise,
  updateExpertise,
} from "../../../../redux/AdminSolutionSlices/ExpertiseSlice";
...
const ExpertiseContactForm = ({ isOpen, onClose, expertiseData, setAlert }) => {
  const { isLoading } = useSelector((state) => state.expertise);
  ...
  const isEditMode = !!expertiseData; // slideData இருந்தால் Edit Mode
  ...
  useEffect(() => {
    if (expertiseData) {
      setFormData({
        ...
        slideOrder: expertiseData.slideOrder,
        objectPosition: expertiseData.objectPosition,
        fontVariant: expertiseData.fontVariant,
      ...
      setPreviewImage(expertiseData.image);
  ...
  const handleSubmit = (e) => {
    ...
    const successMessage = isEditMode
      ...
      : "New expertise successfully created!";

    // 💡 Dispatch செய்ய வேண்டிய action-ஐ தீர்மானிக்கவும்
    const action = isEditMode
      ? updateExpertise({ id: expertiseData.id, formData: apiFormData })
      : createExpertise(apiFormData); // 💡 createSlide action சேர்க்கப்பட்டுள்ளது
...
export default ExpertiseContactForm;
