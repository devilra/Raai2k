...
const AboutBannerForm = ({ isOpen, onClose, aboutBannerData, setAlert }) => {
  // 💡 solutionBanner state-ஐப் பயன்படுத்துகிறோம்
  const { isLoading } = useSelector((state) => state.aboutBanner);
  const [formData, setFormData] = useState(initialFormData);
  const [isImageChange, setIsImageChange] = useState(false);
  const [previewImage, setPreviewImage] = useState(null);
  const dispatch = useDispatch();

  const isEditMode = !!aboutBannerData; // bannerData இருந்தால் Edit Mode
  ...
  useEffect(() => {
    if (aboutBannerData) {
      ...
      setFormData({
        ...
        fontVariant: aboutBannerData.fontVariant,
        isActive: aboutBannerData.isActive,
        image: null,
      });
      setPreviewImage(aboutBannerData.image);
  ...
  const handleSubmit = (e) => {
    ...
    const action = isEditMode
      ? updateAboutBanner({ id: aboutBannerData.id, formData: apiFormData })
      : createAboutBanner(apiFormData); // 💡 createSolutionBanner action
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="mb-6 border p-4 rounded-lg bg-gray-50">
            ...
            <input
              ...
              accept="image/*"
            />
            {(previewImage || (isEditMode && aboutBannerData.image)) && (
              <div className="mt-3 relative w-full h-40 bg-gray-200 rounded-md overflow-hidden">
                <img
                  src={previewImage || aboutBannerData.image}
                  alt="Preview"
