...
import {
  ...
  fetchCeoMessages,
  updateCeoMessage,
} from "../../../../redux/AdminAboutSlices/aboutCeoSlice";
...
const AboutCeoContactForm = ({ isOpen, onClose, ceoData, setAlert }) => {
  const { isLoading } = useSelector((state) => state.ceoContent);
  ...
  useEffect(() => {
    if (ceoData && isOpen) {
      setFormData({
        ...
        ceoName: ceoData.ceoName,
        ceoPosition: ceoData.ceoPosition,
        slideOrder: ceoData.slideOrder,
        // objectPosition: slideData.objectPosition,
        // fontVariant: slideData.fontVariant,
        isActive: ceoData.isActive,
  ...
  const handleSubmit = (e) => {
    ...
    apiFormData.append("ceoName", formData.ceoName); // Append Name
    apiFormData.append("ceoPosition", formData.ceoPosition); // Append Position
    ...
    const successMessage = isEditMode
      ? "CEO Message updated successfully!"
      : "CEO Message created successfully!";

    // 💡 Dispatch செய்ய வேண்டிய action-ஐ தீர்மானிக்கவும்
    const action = isEditMode
      ? updateCeoMessage({ id: ceoData.id, formData: apiFormData })
      : createCeoMessage(apiFormData); // 💡 createSlide action சேர்க்கப்பட்டுள்ளது

    dispatch(action)
      .unwrap() // Thunk Success/Failure-ஐ கையாள
      .then(() => {
        dispatch(fetchCeoMessages());
    ...
    <div className="fixed inset-0  bg-black bg-opacity-50 flex items-center justify-center z-50 pt-12 overflow-y-auto">
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-2xl p-6 relative max-h-[90vh] overflow-y-auto">
        ...
        <h3 className="text-2xl font-bold mb-6 text-gray-800 border-b pb-2">
          {isEditMode ? "Edit CEO Message" : "Create CEO Message"}
        </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  required
                  className="mt-1 block w-full border border-gray-300 rounded-md p-2"
                  placeholder="e.g. Founder, RAAi2K"
                />
              </div>
            </div>
            <label className="block text-sm font-medium text-gray-700">
          ...
          <div className="mb-6 border p-4 rounded-lg bg-gray-50">
            <label className="block text-sm font-medium text-gray-700 mb-2">
              {isEditMode ? "Change Image (Optional)" : "CEO Avatar Image"}
            </label>
            <input
              type="file"
              name="image"
...
export default AboutCeoContactForm;
