...
const WelcomeSection = () => {
  ...
  const {
    welcomeContents, // அனைத்து contents-களும் (List)
    isLoading,
    isError,
    isSuccess,
    message,
  } = useSelector((state) => state.welcomeContent);
  ...
  useEffect(() => {
    ...
    if (isSuccess || isError) {
      dispatch(resetWelcomeContentState());
    }
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this slide?")) {
      dispatch(deleteWelcomeContent(id));
    }
  };
  ...
  if (isLoading && welcomeContents.length === 0) {
        ...
        <div className="bg-white shadow-lg rounded-xl overflow-hidden">
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {welcomeContents.length > 0 ? (
                welcomeContents
                  .filter((welcome) => welcome && welcome.order)
                  .map((welcome) => (
                    <tr key={welcome.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <Button
                          onClick={() => handleEditWelcome(welcome)}
                        ...
                        <Button
                          color="error"
                          onClick={() => handleDelete(welcome.id)}
